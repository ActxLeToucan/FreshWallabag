...
class App {
  ...
  public static async loop() {
    logger.info(
      `== Starting loop with interval of ${RUN_INTERVAL_MINUTES} minutes ==`,
    );
    ...
    try {
      ...
      setInterval(
        () => {
          new App()
            .run()
            .catch((error) => logger.error('Error in scheduled run:', error));
        },
  ...
  public async run() {
    ...
    await this.freshrss.login(
      FRESHRSS_GREADER_API_USER,
      FRESHRSS_GREADER_API_PASSWORD,
    );
    logger.info(' ✅ FreshRSS login successful');
    await this.wallabag.login(
      WALLABAG_CLIENT_ID,
      WALLABAG_CLIENT_SECRET,
      WALLABAG_USERNAME,
      WALLABAG_PASSWORD,
    ...
    const wallabagSubscriptions = subscriptions.subscriptions.filter((sub) =>
      sub.url.match(WALLABAG_URL_PATTERN),
    );
    logger.info(
      ` ✅ Fetched ${subscriptions.subscriptions.length} subscriptions (${wallabagSubscriptions.length} Wallabag feeds)`,
    );
    ...
    for (const sub of wallabagSubscriptions) {
      ...
      for (const article of articles) {
        ...
        const wallabagArticle = wallabagArticles.find(
          (wa) =>
    ...
    logger.info(
      ` ✅ Found ${articlesToMarkAsRead.length} articles to mark as read`,
    );
...
try {
  App.loop()
    .then(() => logger.info('Initial loop completed'))
    .catch((error) => logger.error('Error in initial loop:', error));
} catch (error) {
