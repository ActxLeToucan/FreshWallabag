...
import {
  IWallabagArticle,
  IWallabagArticlesResponse,
} from '@/interfaces/Wallabag.Article.interface';
...
class WallabagService {
  ...
  public async login(
    ...
    return axios
      .post<{ access_token: string }>(url.toString(), {
        grant_type: 'password',
        client_id: clientId,
  ...
  public async getUnreadArticles(
    page: number = 1,
  ): Promise<IWallabagArticle[]> {
    if (!this.auth) {
    ...
    return axios
      .get<IWallabagArticlesResponse>(url.toString(), {
        headers: {
      ...
      .then(async (res) => {
        ...
        const articles: IWallabagArticle[] = content._embedded.items.map(
