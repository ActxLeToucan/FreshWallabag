...
import {
  IFreshRSSArticle,
  IFreshRSSArticlesResponse,
} from '@/interfaces/FreshRSS.Article.interface';
...
class FreshRSSService {
  ...
  public async login(email: string, password: string): Promise<void> {
    ...
    return axios
      .post<string>(url.toString(), formData, {
  ...
  public async getSubscriptions(): Promise<IFreshRSSSubscriptionsResponse> {
    ...
    return axios
      .get<IFreshRSSSubscriptionsResponse>(url.toString(), {
        headers: {
  ...
  public async getArticles(
    ...
    return axios
      .get<IFreshRSSArticlesResponse>(url.toString(), {
        headers: {
          Authorization: this.auth,
        },
      })
      .then(async (res) => {
        ...
        const articles = content.items.map((article) =>
        ...
        return articles.concat(
          await this.getArticles(subscriptionId, content.continuation),
        );
      })
      .catch((err) => {
